...
def get_openai_client():
    ...
    # Access API key directly from Streamlit Secrets
    api_key = st.secrets.get("OPENAI_API_KEY")

    if not api_key:
        st.error("OpenAI API Key not found. Please set it in Streamlit Secrets on the deployment dashboard.")
    ...
    try:
        ...
        st.error(f"Failed to initialize OpenAI client: {e}. Check your API key and internet connection.")
        st.stop()
        ...
        if "weather" in user_input.lower() and "in" in user_input.lower():
            try:
                ...
                if location:
                    ...
                    # We add this as a user message for the LLM to process in the next turn
                    st.session_state.messages.append({"role": "user",
                                                      "content": f"The user asked about weather in {location}. I used the get_current_weather tool and got this result: '{tool_output}'. Please provide a user-friendly response based on this."})
...
@st.cache_resource
def get_agent_instance_for_streamlit():  # Renamed to avoid potential conflict if agent_core.py exists
    return AIAgent(client=openai_client)


agent = get_agent_instance_for_streamlit()
